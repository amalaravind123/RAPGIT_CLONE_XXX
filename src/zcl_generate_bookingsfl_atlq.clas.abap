CLASS zcl_generate_bookingsfl_ATLQ DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_generate_bookingsfl_ATLQ IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbookingfl_ATLQ.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbookingfl_ATLQ.
*   insert the new table entries
    INSERT ztbookingfl_ATLQ FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbookingfl_ATLQ INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
